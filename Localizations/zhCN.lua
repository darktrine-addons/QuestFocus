...
L.BINDING_FOCUS     = "聚焦当前区域"
L.BINDING_FOCUS_ADD = "聚焦当前区域 + 从任务日志添加"
L.BINDING_REVERT    = "恢复追踪"
...
L.CHAT_DOT_HINT                = "队伍状态点会实时更新——将鼠标悬停在任务行上可查看每名队员的进度。颜色和形状：/qf settings。"
...
L.CHAT_MODE_CAMPAIGN    = "仅战役"
L.CHAT_MODE_DAILY       = "仅日常"
L.CHAT_MODE_WEEKLIES     = "仅周常"
L.CHAT_MODE_IMPORTANT   = "仅重要任务"
L.CHAT_MODE_READY       = "仅可交任务"
L.CHAT_MODE_IN_PROGRESS = "仅进行中"

-- ============================================================
-- MODE_ — 模式描述（菜单、鼠标提示）
-- ============================================================

L.MODE_ZONE_FILTER = "当前区域（聚焦）"
L.MODE_TRACK_ALL   = "任务日志中的全部任务"
L.MODE_CAMPAIGN    = "仅战役任务"
L.MODE_DAILY       = "仅日常任务"
L.MODE_WEEKLIES    = "仅周常任务"
L.MODE_IMPORTANT   = "仅重要任务"
L.MODE_READY       = "仅可交任务"
L.MODE_IN_PROGRESS = "仅进行中的任务"
L.MODE_UNTRACK_ALL = "全部取消追踪"
L.MODE_ACTIVE      = "活动中"
...
L.SETTING_ZF_ENABLE_TIP        = "将任务追踪列表筛选为当前区域内有目标的任务，并支持一键恢复。会在任务追踪器和世界地图任务日志中添加两个小按钮。\n\n|cffff8c26此选项的更改需要 /reload 才能生效。|r"
L.SETTING_PS_ENABLE            = "启用队伍同步"
L.SETTING_PS_ENABLE_TIP        = "组队时，在每个追踪任务行上显示彩色状态点；同时在任务行的鼠标提示中添加“队伍状态：”部分，显示每名队员的任务进度。\n\n|cff44ff44立即生效。|r"
...
L.SETTING_ZF_UNTACK_CLEARS_TIP = "开启后，应用筛选器时手动取消追踪的任务，在恢复后不会被重新追踪。按角色保存。\n\n|cffaaaaaa默认关闭：恢复会将任务追踪列表精确恢复到应用筛选器时的状态，即使你之后手动取消了其中一些任务。|r"
L.SETTING_ZF_ZONE_NUDGE         = "区域变更提醒"
L.SETTING_ZF_ZONE_NUDGE_TIP     = "当区域筛选处于活动状态并进入新区域时，让镜头按钮闪烁，并在聊天框中显示带有一键[重新聚焦]链接的提示。在你点击之前，不会重新追踪任何任务。按角色保存。"
...
L.ANCHOR_RIGHT_OF_TITLE = "标题文字右侧"
L.ANCHOR_LEFT_OF_TITLE  = "标题文字左侧"
...
L.SETTING_PS_SIZE             = "状态点大小"
L.SETTING_PS_SIZE_TIP         = "每个追踪任务行上状态点的直径。"
L.SETTING_PS_SHAPE             = "状态点形状"
L.SETTING_PS_SHAPE_TIP         = "状态点的形状。圆形使用圆形透明遮罩覆盖纯色；菱形则将相同纹理旋转45度。"
L.SETTING_PS_ANCHOR            = "状态点位置"
L.SETTING_PS_ANCHOR_TIP        = "设置状态点在每个任务追踪行上的位置。“标题文字左侧”会向左偏移30像素，以避开任务类型图标。如果找不到任务行的标题文字，任一“标题文字”位置选项都会回退到右上角。"
L.SETTING_PS_PALETTE            = "颜色方案"
L.SETTING_PS_PALETTE_TIP        = "为有色觉差异的用户提供替代颜色方案。默认 = 绿色 / 黄色 / 蓝色 / 橙色。"
L.SETTING_PS_OPACITY            = "状态点透明度"
L.SETTING_PS_OPACITY_TIP        = "状态点的亮度。40% 会使其融入背景；100% 为完全不透明。"
L.SETTING_PS_RAID_THRESHOLD     = "大型队伍中汇总鼠标提示"
L.SETTING_PS_RAID_THRESHOLD_TIP = "在大型队伍中，逐个显示成员“队伍状态”的鼠标提示列表会过长。当队伍人数达到阈值后，将其替换为一行汇总：有多少成员正在进行该任务、可以交任务或没有该任务。状态点仍会正常显示。"
L.SETTING_PREVIEW_TRACKER       = "在任务追踪器中预览（30秒）"
L.SETTING_PREVIEW_TRACKER_TIP   = "临时在你的任务追踪行上添加最多4个示例状态点，以便查看当前样式在实际环境中的效果。如果追踪任务少于4个，则相应减少；没有追踪任务时不执行任何操作。再次点击可延长30秒计时；关闭设置面板会结束预览。"
...
L.MENU_DRIFTED         = "（已偏离）"
L.MENU_TITLE           = "任务追踪模式"
L.MENU_TRACK_ALL       = "追踪任务日志中的全部任务"
L.MENU_FOCUS           = "追踪当前区域（聚焦）"
L.MENU_FOCUS_ADD       = "追踪当前区域 + 从任务日志添加"
L.MENU_CAMPAIGN        = "仅追踪战役任务"
L.MENU_DAILY           = "仅追踪日常任务"
L.MENU_WEEKLIES        = "仅追踪周常任务"
L.MENU_IMPORTANT       = "仅追踪重要任务"
L.MENU_READY            = "仅追踪可交任务"
L.MENU_IN_PROGRESS      = "仅追踪进行中的任务"
L.MENU_UNTRACK_ALL      = "取消追踪所有任务"
L.MENU_OPEN_SETTINGS    = "打开设置……"
L.MENU_WARN_FOOTER      = "此操作会清空任务追踪器（任务将被隐藏）。"
...
L.KEY_LEFT_CLICK           = "左键点击"
L.KEY_SHIFT_LEFT_CLICK     = "Shift+左键点击"
L.TIP_KEY_LEFT_CLICK       = "左键点击"
L.TIP_KEY_SHIFT_LEFT_CLICK = "Shift+左键点击"
...
L.TIP_NARROW_DESC          = "将任务追踪列表缩小为当前区域内有目标的任务。"
L.TIP_NO_CHANGES           = "（无变化）"
L.TIP_MAP_UNKNOWN           = "当前区域未知——请打开一次世界地图。"
L.TIP_FOCUS_LINE            = "%s：聚焦当前区域 %s |cffaaaaaa（追踪 %d）|r%s"
L.TIP_FOCUS_ADD_LINE        = "%s：聚焦当前区域 + 从任务日志添加 %s |cffaaaaaa（追踪 %d）|r%s"
L.TIP_REAPPLY_HINT          = "|cff44ff44绿色按钮|r|cffaaaaaa 会重新应用当前模式。|r"
L.TIP_WARN_LEGEND           = "= 任务追踪器将隐藏"
L.TIP_HOTKEYS               = "右键：模式菜单  |  Shift+右键：设置"
L.TIP_RESTORE_TITLE          = "恢复任务追踪"
...
L.PARTY_STATE             = "队伍状态："
L.PARTY_YOU               = "你"
L.PARTY_DOT_LEGEND        = "状态点：  %s   %s   %s   %s"
